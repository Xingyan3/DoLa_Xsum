...
class DoLa:
    ...
    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None, premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True, remove_stop_words=False, relative_top=0.1, **kwargs):
        with torch.no_grad():
            ...
            input_ids = self.tokenizer.encode(input_text, return_tensors="pt", max_length=4096, truncation=True).to(self.device)
            ...
            if mode == 'baseline':
                ...
                outputs = self.model.generate(input_ids, max_length=max_len, num_return_sequences=1,
                                        output_scores=True, return_dict_in_generate=True, dola_decoding=True, output_hidden_states=True,
            ...
            print(len(gen_sequences), "hhhhhhhhhhhh")

            # skip the tokens in the input prompt
            # gen_sequences = sequences[:, input_ids.shape[-1]:][0, :]
            # gen_arr = gen_sequences.cpu().numpy()

            output_str = self.tokenizer.decode(gen_sequences, skip_special_tokens=False)
            for i in range(gen_sequences.size(0)):
                print(gen_sequences[i])
                print(self.tokenizer.decode(gen_sequences[i], skip_special_tokens=False))
